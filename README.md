...
DB가 아예 없는 상태에서 시작할 때의 전체 순서다. 이미 DB가 있으면 4번만 하면 된다.

**1. MySQL 8 설치 후 빈 DB를 만든다.** `schema.sql`은 테이블만 만들고 DB는 만들지 않는다.

```sql
CREATE DATABASE wallet DEFAULT CHARSET utf8mb4 COLLATE utf8mb4_0900_ai_ci;
```

**2. 접속 설정 파일을 만든다.** `db.properties`는 각자 환경 값이라 저장소에 없다. 예시를 복사해 채운다.
...
`db.url`의 DB 이름, `db.username`, `db.password`, `app.image.storage-path`를 자기 값으로 바꾼다.
`app.image.storage-path`가 비어 있으면 플레이스홀더가 풀리지 않아 **애플리케이션이 기동하지 않는다.**

**3. 카드 이미지 폴더를 만든다.** 2번에 적은 경로 아래에 `cards/` 를 만들고 이미지를 넣는다.

```
<app.image.storage-path>/cards/*.png
```

**4. DB 스크립트를 순서대로 실행한다.** (아래 표)

**5. 빌드하고 띄운다.**

```bash
cd backend && ./mvnw clean package -DskipTests    # target/ROOT.war 를 Tomcat에 배포
