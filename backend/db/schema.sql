...
-- ------------------------------------------------------------
-- 통합 ERD(docs/_private/wallet-erd-merged.dbml)와 1:1로 대응한다.
...
--   · 엔진 도메인의 enum 성격 컬럼은 VARCHAR + COMMENT
--     (MySQL ENUM은 값 추가 시 ALTER가 필요하고 Java enum과 이중 관리가 된다)
--   · 회원·알림 도메인은 담당자 설계를 존중해 MySQL ENUM을 그대로 둔다.
--     서로 조인·비교하지 않아 스타일 차이의 실제 영향이 없다
...
-- ════════════════════════════════════════════════════════════
-- 8. 엔진 상태 (결제마다 갱신되고, 그 갱신이 다음 추천을 바꾼다 = 동적 전환)
--
