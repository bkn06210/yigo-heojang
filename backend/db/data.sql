...
-- simple_password_hash: 간편비밀번호 '123456'의 BCrypt 해시다.
-- 결제 화면이 이 값으로 실제 검증을 하므로, 데모 계정에는 미리 설정해 둔다.
-- 설정하지 않은 회원(NULL)은 결제 시 "간편비밀번호를 먼저 설정하세요" 안내를 받는다.
INSERT INTO member (
    member_id, email, password_hash, simple_password_hash, name, nickname, member_status,
    created_at, updated_at, withdrawn_at
) VALUES
    (1, 'active@example.com',
     '$2a$10$Exc5x/juU54pWSz1Wo8E7Opgva7S.1W5uq6in1D/BH.krG3gKs93C',
     '$2a$10$ecygc6FyzyxHoUi8twQNJe8Qk1sXaIX7.Vpc5wPe6cjpHXa0WvWMy',
     '김활성', '별명A', 'ACTIVE',
    ...
    -- 실제 탈퇴 서비스 코드(MemberService.withdraw)가 채워 넣을 값의 형태를 그대로 흉내 낸다.
...
-- 보관 목적으로 옮겨 담은 행이다. 실제 서비스에서는 회원 탈퇴 트랜잭션이
-- 익명화 UPDATE 직전에 원본을 읽어 이 테이블에 INSERT한다.
-- email_hash는 'withdrawn@example.com'의 SHA-256 hex 값이다.
...
INSERT INTO term (
    ...
    (4, 'LOCATION_TERMS', 'SIGNUP', '위치정보 이용약관', 0, 'INACTIVE'),

    -- 회원 탈퇴 화면에서 고지하고 동의를 받는 약관이다.
    -- is_required = 1인 이유: 탈퇴는 신용정보 즉시 삭제·개인정보 보관이라는
    -- 되돌릴 수 없는 처리가 뒤따르므로, 이 고지에 동의하지 않으면 탈퇴 자체를 진행할 수 없다.
    (5, 'WITHDRAWAL_NOTICE', 'WITHDRAWAL', '회원 탈퇴 안내 및 동의', 1, 'ACTIVE');
...
INSERT INTO member_term_agreement (
    ...
    -- member_id=3은 탈퇴할 때 WITHDRAWAL_NOTICE 약관에 동의했다.
    (7, 3, 5, 1, '2026-07-10 14:00:00');
