...
import com.wallet.member.dto.PasswordVerifyRequest;
...
public class MemberService {
    ...
    // 어떤 원문으로도 BCrypt 매칭이 성립할 수 없는 값이라, 탈퇴 후에는 로그인 자체가 불가능해진다.
    ...
    @Transactional(readOnly = true)
    public void verifyPassword(Long memberId, PasswordVerifyRequest request) {
        validateAuthenticatedMemberId(memberId);
     ...
     * 회원 탈퇴를 처리한다.
     * <p>
     * 데이터 처리 방침이 세 갈래로 나뉜다.
     * <ul>
     *   <li>신용/거래 정보(카드·결제·소비·포인트)와 알림·개인화 설정: 즉시 물리 삭제</li>
     *   <li>이메일·성명 등 개인정보: member 테이블에서는 마스킹하고
     *       원본은 member_withdrawal_archive로 옮겨 보관 기간 동안만 유지</li>
     *   <li>member 행 자체와 약관 동의 이력: 보존
     *       (member_term_agreement가 member를 FK로 참조하므로 행을 지울 수 없다)</li>
     * </ul>
     * 전체가 하나의 트랜잭션이라 중간에 실패하면 아무것도 지워지지 않는다.
     */
    @Transactional
    public void withdraw(Long memberId, MemberWithdrawRequest request) {
        validateAuthenticatedMemberId(memberId);

        // 1. 회원 행을 잠근다. 대표 카드 변경 로직과 동일한 조건(ACTIVE + withdrawn_at IS NULL)이라,
        // 이미 탈퇴한 회원이 다시 탈퇴를 시도하면 여기서 null을 반환해 MEMBER_NOT_FOUND로 막힌다.
        Long lockedMemberId = memberMapper.lockActiveMemberById(memberId);
        if (lockedMemberId == null) {
            throw new BusinessException(ErrorCode.MEMBER_NOT_FOUND);
        }

        // 2. 익명화되기 전에 원본 정보를 읽어둔다. 이 시점 이후로는 member 테이블의
        // email·name을 다시 조회해도 마스킹된 값만 나오므로, archive에 넣을 원본은 지금 확보해야 한다.
        Member member = memberMapper.findById(memberId);
        if (member == null) {
            throw new BusinessException(ErrorCode.MEMBER_NOT_FOUND);
        }
        ...
        // member_withdrawal_archive.withdrawn_at 전부 이 값을 기준으로 삼는다.
        LocalDateTime withdrawnAt = LocalDateTime.now();

        // 3. 탈퇴 고지 약관에 동의했다는 이력을 남긴다. 회원가입과 같은
        // insertMemberTermAgreements를 재사용한다 — 원소 하나짜리 리스트를 넘기면 동일하게 동작한다.
        termAgreementMapper.insertMemberTermAgreements(
            List.of(new MemberTermAgreement(memberId, request.termVersionId(), true))
        );

        // 4. 알림·개인화 관련 데이터를 가장 먼저 지운다. notification이 아래 신용정보 그룹의
        // point_history·user_card를 FK로 참조하고 있어서, 반드시 이 그룹이 신용정보 삭제보다 먼저 끝나야 한다.
        ...
        memberDataCleanupMapper.deleteSimplePasswordVerificationByMemberId(memberId);
        ...
        // 6. member 자신을 익명화한다. "다른 도메인 정리 → 회원 자신 처리" 순서가 자연스러워 이 위치에 둔다.
        ...
        memberWithdrawalArchiveMapper.insert(
            MemberWithdrawalArchive.of(
                memberId,
                member.getEmail(),
                tokenHashUtil.sha256(member.getEmail()),
                member.getName(),
                withdrawnAt,
                RETENTION_REASON
            )
        ...
        // 9. 모든 활성 세션을 종료한다. 데이터 정리가 전부 끝난 뒤 마지막에 실행해,
        // 트랜잭션 중간에 실패하더라도(전체 롤백되므로) 세션이 먼저 끊기고
        // 데이터는 안 지워지는 어중간한 상태가 남지 않는다.
    ...
    // 프론트가 제출한 termVersionId가 "지금 실제로 유효한 탈퇴 고지 약관"이 맞는지 검증한다.
    // 탈퇴 약관 조회 API가 내려준 값을 그대로 믿지 않는 이유는, 조회 시점과
    // 탈퇴 실행 시점 사이에 약관이 개정될 수 있기 때문이다.
