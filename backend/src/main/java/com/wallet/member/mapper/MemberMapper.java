...
public interface MemberMapper {
    ...
    /**
     * JwtAuthenticationFilter가 매 요청마다 "이 회원이 지금 탈퇴/정지 상태인지"만
     * 확인하기 위한 조회 쿼리.
    ...
    /**
     * 회원 탈퇴 시 member 행을 물리 삭제하지 않고 개인정보만 마스킹 값으로 치환한다.
     * <p>
     * member 행을 남기는 이유는 member_term_agreement가 member를 FK로 참조하고 있어
     * 약관 동의 이력을 보존하려면 회원 행 자체가 남아 있어야 하기 때문이다.
     * 마스킹 값 자체는 서비스 계층에서 미리 계산해 넘겨받는다.
     */
    int anonymizeMember(
