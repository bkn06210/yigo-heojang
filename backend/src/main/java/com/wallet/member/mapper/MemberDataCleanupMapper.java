...
// 회원 탈퇴 시 신용정보가 아닌 나머지 데이터(알림, 개인화 설정, 인증 부가정보 등)를 즉시 물리 삭제하는 매퍼
@Mapper
public interface MemberDataCleanupMapper {
    ...
    // 아래 5개는 서로 순서 제약이 없다. member_personalization_brand가
    // member_personalization_category를 참조하고 있어(ON DELETE CASCADE 있음)
    // category보다 brand를 먼저 지우는 것만 지키면 된다.
    ...
    // brand가 category를 참조한다(ON DELETE CASCADE 있음). category만 지워도
    // DB가 brand를 자동으로 같이 지워주지만, 이 스키마 전체에서 CASCADE를 쓰는
    // 곳이 여기 한 곳뿐이라 다른 삭제들과 마찬가지로 명시적으로 먼저 지운다.
