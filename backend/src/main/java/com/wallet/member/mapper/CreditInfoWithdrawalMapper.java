...
public interface CreditInfoWithdrawalMapper {
    // 아래 11개는 반드시 이 순서(선언 순서가 아니라 실제 호출 순서)로
    // 실행해야 한다. FK 참조 관계상 자식 테이블을 부모 테이블보다 먼저
    // 지워야 하기 때문이다.
    //
    // 주의: 이 11개를 실행하기 전에 MemberDataCleanupMapper의
    // notification 삭제가 반드시 먼저 끝나 있어야 한다. notification이
    // 이 그룹의 point_history·user_card를 FK로 참조하고 있어서다.
    // 이 제약은 매퍼 두 개에 걸쳐 있어 어느 한쪽 인터페이스만 봐서는
    // 알 수 없으므로, 실제 호출 순서는 서비스 계층에서 명시적으로 관리한다.
