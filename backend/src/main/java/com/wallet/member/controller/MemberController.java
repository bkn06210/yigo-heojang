...
public class MemberController {
    ...
    // 간편비밀번호는 로그인 상태만으로 바꿀 수 없다. 회원 본인의 이메일로 인증 코드를
    // 보내고, 그 코드를 맞힌 요청에만 변경 토큰을 발급한다.
    // 메일 발송은 성공했지만 아직 인증이 끝난 건 아니므로 202 Accepted로 응답한다.
    ...
    // 최초 설정과 기존 간편비밀번호 변경 모두 이 API를 쓴다. 결과 상태가 하나로 정해지므로
    // POST가 아니라 PUT이다.
    ...
    // 결제 등에서 본인 확인용으로 호출한다. 일치 여부만 matched로 돌려준다.
    ...
    @PostMapping("/me/password/verifications")
    public ResponseEntity<ApiResponse<Void>> verifyPassword(
        HttpServletRequest request,
        @Valid @RequestBody PasswordVerifyRequest verifyRequest
    ) {
        Long memberId = (Long) request.getAttribute(AUTHENTICATED_MEMBER_ID);

        memberService.verifyPassword(memberId, verifyRequest);

        return ResponseEntity.ok(
            ApiResponse.success("비밀번호 확인이 완료되었습니다.", null)
        );
    }

    // 탈퇴 화면에 진입했을 때 프론트가 고지 문구와 termsVersionId를 받아가는 API.
    // 이 termsVersionId를 사용자가 "동의" 체크 후 DELETE /api/members/me 요청의
    // termVersionId로 그대로 담아 보내면, 그 값이 지금 실제로 유효한 WITHDRAWAL 스코프
    // 약관 버전이 맞는지 서버가 한 번 더 검증한다(프론트가 보낸 값을 그대로 믿지 않는다).
    //
    // 회원 정보를 조회하는 게 아니라 "지금 유효한 약관이 무엇인지"만 필요하므로
    // HttpServletRequest에서 memberId를 꺼내 쓰지 않는다. 다만 이 API 자체는
    // JwtAuthenticationFilter의 인증 대상 경로에 포함되어 있어, 로그인하지 않은 상태로는 호출할 수 없다.
    ...
    // 응답 바디에 돌려줄 데이터가 없어 ApiResponse<Void>로 응답한다.
    @DeleteMapping("/me")
