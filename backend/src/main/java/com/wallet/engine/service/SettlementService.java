...
public class SettlementService {
    ...
    public PaymentSettlementResult applyPayment(SettlementCommand command) {
        ...
        List<PerformanceTier> tiers = performanceInputAssembler.toTiers(
                performanceMapper.findTiers(command.cardId()));
        ...
        List<BenefitRow> benefitRows = benefitMapper.findActiveBenefits(command.cardId(), status.tierId());
        ...
        CardState cardState = loadCardState(command.userCardId(), baseYearMonth, status, appliedDate);
    ...
    private CardState loadCardState(long userCardId, String baseYearMonth,
                                    PerformanceStatus status, LocalDate appliedDate) {
        MonthlyStatusRow baseRow = settlementMapper.findMonthlyStatus(userCardId, baseYearMonth);
        ...
        return new CardState(
                status.performanceMet(), status.sharedMonthlyLimit(), sharedLimitUsed,
                cardStateAssembler.toUsages(usageRows, appliedDate));
    }
    ...
    private CardMonthlyStatus buildStatus(long userCardId, long cardId, String baseYearMonth) {
        ...
        List<PerformanceTier> tiers = performanceInputAssembler.toTiers(
                performanceMapper.findTiers(cardId));
        long tierId = tierResolver.judge(tiers, statusRow.getPrevPerformanceAmount()).tierId();
        List<BenefitRow> benefitRows = benefitMapper.findActiveBenefits(cardId, tierId);
