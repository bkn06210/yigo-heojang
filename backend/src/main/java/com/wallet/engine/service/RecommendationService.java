...
public class RecommendationService {
    ...
    public RecommendationResponse recommend(long memberId, RecommendationRequest request, LocalDate today) {
        long expectedAmount = resolveExpectedAmount(memberId, request, today);
    ...
    /** 금액을 넣지 않은 회원의 결제 이력을 볼 기간. 너무 길게 잡으면 지금 씀씀이와 멀어진다 */
    private static final int AVERAGE_LOOKBACK_MONTHS = 3;

    /**
     * 이력이 하나도 없을 때 쓰는 금액.
     *
     * 갓 가입해 결제 기록이 없는 회원도 추천 화면을 볼 수 있어야 한다.
     * 이 값으로는 "건당 5만원 이상" 류의 조건이 걸린 혜택이 잡히지 않지만,
     * 아무것도 못 보여주는 것보다 낫다.
     */
    private static final long FALLBACK_AMOUNT = 30_000L;

    /**
     * 계산에 쓸 금액을 정한다.
     *
     * 회원이 넣었으면 그 값이다. 넣지 않았으면 본인의 최근 평균 결제액으로 채운다 —
     * 엔진은 금액 없이 계산할 수 없는데(할인 한도·건당 최소금액·적립률이 전부 금액에 걸린다),
     * 그렇다고 고정 상수를 쓰면 그 사람 소비와 무관한 추천이 된다.
     *
     * 업종을 골랐으면 그 업종 평균을 먼저 본다. 카페를 고른 사람에게 주유 평균을 적용하면 어긋난다.
     * 그 업종 이력이 없으면 업종을 가리지 않은 평균, 그것도 없으면 기본값 순으로 물러난다.
     *
     * 0이나 음수는 회원이 잘못 넣은 값이라 평균으로 바꾸지 않고 그대로 막는다.
     */
    private long resolveExpectedAmount(long memberId, RecommendationRequest request, LocalDate today) {
        Long requested = request.getExpectedAmount();
        if (requested != null) {
            if (requested <= 0L) {
        ...
        LocalDateTime from = today.minusMonths(AVERAGE_LOOKBACK_MONTHS).atStartOfDay();
        Long average = performanceMapper.findAverageExpenseAmount(memberId, request.getCategoryId(), from);
        if (average == null || average <= 0L) {
            average = performanceMapper.findAverageExpenseAmount(memberId, null, from);
        }
        return (average == null || average <= 0L) ? FALLBACK_AMOUNT : average;
    }
