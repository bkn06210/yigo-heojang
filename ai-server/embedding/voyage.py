...
글을 쓰는 모델과 달리 값만 내주므로 훨씬 싸다. 계정당 무료 2억 토큰이 있고
약관 색인 전체가 130만 토큰 규모라 요금이 붙는 구간에 들어가지 않는다.

한국어 성능은 아직 확인하지 못했다. 공급자를 갈아탈 수 있게 어댑터 뒤에 둔 이유다.

호출 주소는 설정으로 받는다. 키를 발급받은 곳에 따라 주소가 달라지는데, 요청·응답 모양은
같아서 구현을 나눌 이유가 없다. 주소가 어긋나면 인증 단계에서 401 로 떨어진다.
"""
