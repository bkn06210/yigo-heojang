...
# 업종 낱말을 의도 규칙에도 쓴다. 표를 늘리면 규칙이 함께 늘어나 따로 관리할 일이 없다.
_CATEGORY_PATTERN = "|".join(_CATEGORY_WORDS)
...
_INTENT_RULES: Tuple[Tuple[str, str], ...] = (
    ...
    # "카드"라는 낱말 없이 결제할 자리만 말하는 것이 오히려 자연스럽다("스벅 가는데 뭐 쓸까").
    # 사람은 뜻으로 알아듣지만 이 규칙은 글자만 보므로, 분류 프롬프트에 예시로 적어둔
    # 말투를 여기에도 적어 둔다. 개발 중에 대표 질문이 UNKNOWN 으로 빠지면 멀쩡한 기능을
    # 고장 난 것으로 오해하게 된다.
    (IntentName.RECOMMEND_CARD,
     r"어느 ?카드|어떤 ?카드|무슨 ?카드|뭘로|추천"
     r"|뭐가 ?좋|뭐 ?좋|뭐 ?쓸|뭐로|긁을|결제할"),
    (IntentName.CARD_STATUS, r"실적|한도|남은|달성|채웠|채워"),
    (IntentName.TERM_QA, r"약관|청구|언제 빠져|분실|잃어버|도난|해지|없애|연회비|재발급|수수료"),
    # 마지막 수단 둘 — 금액만 말하거나("올영에서 3만원") 업종만 말하는("지하철 탈 건데")
    # 질문이다. 앞의 규칙이 전부 안 걸렸을 때만 닿으므로 다른 의도를 가로채지 않는다.
    (IntentName.RECOMMEND_CARD, r"\d[\d,]*\s*(?:만)?원"),
    (IntentName.RECOMMEND_CARD, _CATEGORY_PATTERN),
)
